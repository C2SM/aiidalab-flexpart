...
class SearchCalculations(widgets.VBox):
    def __init__(self):
        ...
        search_crit = widgets.HBox([self.location,
                                    widgets.VBox([widgets.HBox([self.model,
                                                                self.model_offline]),
                                                  widgets.HBox([self.outgrid,
                                                                self.outgrid_nest]),
                                                  ad_q])
                                    ])
        ...
        tabs = widgets.Tab(children=[widgets.HBox([self.date_range,
                                                   self.presettings]),
                                    search_crit
                                        ])
    ...
    def search(self):
        ...
        self.w_checkboxes = [widgets.Checkbox(description = f'{i}',style=style) for i in self.w_options]
        ...
        df_for_calendar.index = df_for_calendar.index.map(lambda x : datetime.strptime(x, '%Y-%m-%d %H:%M:%S'))
        
        range_date = pd.date_range(start=date_range_p[0], end=date_range_p[-1],freq ='D')
